...
void loop() {
  ...
  BlinkLed(2, 40, 10);
  float  OnOff = readTSData(channelID, Field6 );
  Serial.print("Field6= ");
  Serial.println(OnOff);
  float  CteCalib1 = readTSData(channelID, Field7 );
  Serial.print("Field7= ");
  Serial.println(CteCalib1);
  float  CteCalib2 = readTSData(channelID, Field8 );
  Serial.print("Field8= ");
  ...
  int veletaValue = analogRead(VELETA_PIN);
  float windDirection = map(veletaValue, 0, 4095, 0, 359);
  Serial.print("Veleta=");
  Serial.println(windDirection);
  digitalWrite(LedPin, LOW);

  
  // Reinicio del contador de RPM
  RPMTops = 0;
